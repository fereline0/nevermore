                ...
                status === "authenticated" &&
                    ...
                    session?.user.role.abilities.map((ability: any) => {
                        if (ability.slug == "deleteUser") {
                            return (
                                <Action name="Delete user" description="You are sure" func={async () => await deleteUser(props.id).then(() => { router.refresh() })} />
                            )
                        }
                        if (ability.slug == "ban") {
                            return (
                                <Action name="Ban" description="You are sure" func={async () => await deleteUser(props.id).then(() => { router.refresh() })} />
                            )
                        }
                    })
            }
